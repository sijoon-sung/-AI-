...
# 전역 변수 설정
GOAL_FILE = "data/goal.json" # 설정된 목표, 할 일을 저장할 JSON 파일 경로
...
def load_goal():
    ...
    with open(GOAL_FILE, encoding="utf-8") as f:
        d = json.load(f)
    return d["subject"], d["todo"] # 과목명 / task return


# 구글 task에서 목표를 선택
def pick_goal():
    print(" Google task 목표 입력")
    print()
    lists = getsubjectlist() #apis - google task에서 가져옴
    #과목 goal 리스트를 가져옴

    # 과목(goal) 목록 출력 ---> 번호랑 함께 출력
...
# AI 피드백 메시지 깔끔하게 출력하는 함수
def print_feedback(res):
...
def timer_thread():
    global last_check_time # # 마지막 점검 시간
    while running:
        ...
        if time.time() - last_check_time >= interval:
            last_check_time = time.time()
            res = startcheck(goal, task_titles) #runagent
            print_feedback(res) # 피드백
            print_status() # status 출력
...
def main():
    ...
    while running:
        ...
        if cmd == "q": # quit
            ...
            print_feedback(res) # 피드백을 출력하는 부분은 따로 함수로
