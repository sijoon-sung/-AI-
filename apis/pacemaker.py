...
def restpopup(goal, reason):
    ...
    for i in apps[:10]:
        timelinelines.append(f"- {i['minutes']}분 사용 | {i['app']} | {i['title'][:50]}")
    timelinetext = "\n".join(timelinelines)

    #  Gemini에 이탈 심리 분석 요청
    prompt = f"""너는 인지심리학 전문 AI야. 주어진 정보를 바탕으로 학생이 집중을 잃은 '트리거' / '경로'를 객관적인 어조로 분석해줘. 4줄 내외로 간결하게 작성해줘. md가 아닌 일반텍스트 형식으로 작성해줘.
[원래 목표]: {goal}
[이탈 원인]: {reason}
[타임라인]:\n{timelinetext}"""
    ...
    # 다시 집중할지 말지 선택
    isfocus = messagebox.askyesno("집중 복귀", "지금 바로 원래 목표로 복귀하시겠습니까?\n('아니오'를 누르시면 5분간 휴식 모드로 진입.)")
...
if __name__ == "__main__":
    restpopup("파이썬 프로그래밍 및 데모 완성", "유튜브 접속 감지됨")